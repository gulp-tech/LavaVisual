...
public final class ClickGuiScreen extends Screen {
    private int page, left, top, panelWidth, panelHeight, row;
    ...
    public ClickGuiScreen(int page) { super(Component.literal("LavaVisual HUD")); this.page = Math.max(0, Math.min(2, page)); }
    private void button(String text, int x, int y, int width, Runnable action) {
        addRenderableWidget(Button.builder(Component.literal(text), b -> action.run()).pos(x, y).size(width, Math.min(20, row - 2)).build());
    }
    private void changed() { LavaVisualClient.save(); rebuildWidgets(); }
    @Override protected void init() {
        panelWidth = Math.min(570, width - 12); panelHeight = Math.min(300, height - 8);
        row = Math.max(14, (panelHeight - 76) / 8);
        left = (width - panelWidth) / 2; top = Math.max(4, (height - panelHeight) / 2);
        ...
        button("Done", left + panelWidth - 78, top + panelHeight - 25, 70, this::onClose);
    }
    private void initHud() {
        ...
        button("HUD: " + (c.enabled ? "ON" : "OFF"), x, top + 60, w, () -> { c.enabled = !c.enabled; changed(); });
        button("Theme: " + (c.dark ? "Dark" : "Glass"), x, top + 60 + row, w, () -> { c.dark = !c.dark; changed(); });
        button("Shadows: " + c.shadows, x, top + 60 + 2 * row, w, () -> { c.shadows = !c.shadows; changed(); });
        button("Animation: " + c.animations, x, top + 60 + 3 * row, w, () -> { c.animations = !c.animations; changed(); });
        button("Crosshair: " + new String[]{"Vanilla", "Dot", "Plus", "Square"}[c.crosshair], x, top + 60 + 4 * row, w,
                () -> { c.crosshair = (c.crosshair + 1) % 4; changed(); });
        button("Crosshair color", x, top + 60 + 5 * row, w, () -> { c.crosshairColor = (c.crosshairColor + 1) % HudConfig.COLORS.length; changed(); });
        button("Drag HUD elements", left + 8, top + 60 + 6 * row, half - 16, () -> minecraft.gui.setScreen(new HudEditorScreen(this)));
        button("Stopwatch: start/pause", x, top + 60 + 6 * row, w, () -> LavaVisualClient.STATE.toggleTimer());
        button("Reset layout", left + 8, top + 60 + 7 * row, half - 16, () -> { LavaVisualClient.resetLayout(); changed(); });
        button("Reset stopwatch", x, top + 60 + 7 * row, w, () -> LavaVisualClient.STATE.reset());
    }
    private void initProfiles() {
        for (int slot = 1; slot <= 3; slot++) {
            final int n = slot;
            int y = top + 65 + (slot - 1) * 2 * row;
    ...
    private void initFriends() {
        ...
        for (int i = 0; i < 5 && friendPage * 5 + i < friends.size(); i++) {
            ...
            button(value + "  [remove]", left + 12, top + 90 + i * row, panelWidth - 24, () -> {
                LavaVisualClient.config().friends.remove(value); changed();
            });
        }
        button("Previous", left + 12, top + panelHeight - 50, 90, () -> { friendPage = Math.max(0, friendPage - 1); rebuildWidgets(); });
        button("Next", left + 108, top + panelHeight - 50, 90, () -> { friendPage++; rebuildWidgets(); });
    }
    @Override public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        HudRenderer.panel(g, left, top, panelWidth, panelHeight, 0xFFFF783E);
