# LavaVisual 2.18.0 — Minecraft 26.2

<img src="ports/mc26.2/src/main/resources/assets/lavavisual/icon.png" width="96" alt="LV">

**[Скачать lavavisual-2.18.0-mc26.2.jar](artifacts/lavavisual-2.18.0-mc26.2.jar)**

![Объёмные крылья · сзади, три четверти, сбоку, спереди](artifacts/wings-2.14-preview.jpg)
![Модели шляп и крыльев](artifacts/models-2.14-preview.jpg)
![Крылья](artifacts/ui-2.14-wings.png)
![Шляпы](artifacts/ui-2.14-hats.png)
![Меню и водянка](artifacts/ui-2.14-menu.png)

## Что нового в 2.18

- **Миникарта больше не роняет FPS**: рамка, круглый вырез, тень и стрелка рисуются одной готовой текстурой
  (в 2.17 это были тысячи мелких прямоугольников за кадр). Автотест проверяет время отрисовки.
- **Свои звуки работают**: папки создаются сами — `config/lavavisual-hud/sounds/hits`, `crits`, `totems`, `kills`
  (файлы прямо в `sounds` — во всех списках). Каждый файл виден в списке под своим именем со звёздочкой, имя любое
  (русские буквы, пробелы). Звуки играют напрямую через OpenAL — без ресурспака и перезагрузки, «Слушать» работает сразу,
  ванильный звук при этом глушится. Файлы можно перетащить прямо в окно игры.
- **Музыкальный плеер** (клавиша M): своя папка `config/lavavisual-hud/music`, пауза, перемотка (полоса и ±10 сек),
  предыдущий / следующий, перемешивание, повтор, громкость, список треков, обложка из файла или картинки рядом.
  Все действия — во вкладке «Бинды». Новая вкладка меню «Музыка».
- **HUD музыки**: обложка или крутящийся диск, название, исполнитель, время, предыдущий и следующий трек; скрывается,
  когда ничего не играет; размер, прозрачность и место настраиваются как у других модулей.
- Формат — `.ogg` (Vorbis); файлы Opus в `.ogg` помечаются «не Vorbis».

## Что нового в 2.17

- **Зум** (удерживать C, колесо меняет приближение): плавный, мышь замедляется пропорционально, рука и HUD не меняются.
- **FreeLook** (удерживать Left Alt): камера облетает вас от третьего лица, а взгляд, прицел и движение остаются на месте;
  после отпускания возвращается прежний вид. Только камера — на сервер ничего не уходит. Клавиши меняются во вкладке «Бинды».
- **Насыщенный крит**: в несколько раз больше ванильных искр крита, цветной взрыв звёзд (свой цвет во вкладке «Цвета»),
  по желанию магические искры и эффект на каждый удар.
- **Звук удара полностью заменяет ванильный**: пока включён ваш звук удара, ванильный звук урона цели и «удар без урона»
  не играют; без отдельного звука крита на крите тоже играет ваш звук удара.
- **Шляпы сидят на голове**: шляпы, которые были уже головы, расширены и чуть опущены, поэтому охватывают голову или шлем
  и не висят над ней (нимб и кристалл по-прежнему парят).
- **Trails с настройками**: 5 стилей (лента, неон, спираль, искры, комета), длина, толщина, яркость и насыщенное
  аддитивное свечение; цвет — градиент от тела к хвосту.
- **Новая миникарта**: круглое окно с градиентной рамкой, мягким свечением и внутренней тенью, гладкая стрелка, круглые метки,
  никаких букв; форму можно переключить на квадрат.
- **FPS Boost сильнее**: дополнительно смешивание биомов 1, дальность симуляции до 8 и ванильный лимит FPS в AFK
  (всё возвращается при выключении).

## Что нового в 2.16

- **Шляпы жёстко закреплены на голове.** Шляпа и крылья теперь рисуются как слой самой модели игрока
  (как ванильный шлем), в том же проходе и с теми же трансформациями головы и тела. Шляпа больше не отстаёт
  при резких и плавных поворотах, крылья идут за корпусом при ударе и приседании.
- **Trails выходят из тела**, а не из точки между ног: мягкая светящаяся лента на уровне корпуса,
  яркая в центре и сужающаяся к хвосту.
- **Редактор рук починен.** Раньше ползунки ничего не меняли, пока был выключен переключатель
  «Положение рук»; теперь редактор включает его сам. Добавлены наклон, поворот и крен руки,
  тонкая подстройка колёсиком мыши над ползунком.
- **Манекен** («Эффекты → Манекен» или бинд «Манекен»): локальный игрок-манекен перед вами, которого видите только вы.
  На нём ваш скин, броня, предметы, шляпа и крылья; можно включить вращение. По нему можно бить: он краснеет,
  теряет здоровье, звучат звуки удара и крита, работают Target HUD, частицы и эффект убийства, после «смерти» он встаёт.
  Сервер о манекене не знает: удары и клики по нему не отправляют пакетов (взмах рукой такой же, как по воздуху).

## Что нового в 2.15

- **Шляпы и крылья больше не дёргаются.** Раньше детализация моделей зависела от текущего FPS
  и раз в секунду пересчитывалась: при FPS около 40–50 сетка модели перестраивалась, и шляпа
  с крыльями «прыгали». Теперь детализация постоянная (с FPS Boost чуть ниже, но тоже постоянная).
- **Крылья поворачиваются вместе с корпусом при ударе**, как у ванильной модели, а не отстают от тела.
- **Шляпа приподнята над слоем скина**, поэтому её низ не мерцает на волосах.
- В CI-тест добавлен настоящий мир: клиент создаёт плоский мир и снимает крылья и шляпы от третьего лица
  (`artifacts/ui-2.15-world_*.png`).

## Что нового в 2.14

- **Объёмные крылья.** Все пять моделей собраны заново из изогнутых поверхностей с гладкими нормалями вместо плоских полосок:
  - перья выгнуты вокруг стержня и повёрнуты так, что ложатся друг на друга черепицей;
  - ряды перьев (маховые, три ряда кроющих, крылышко у запястья) лежат слоями на толстой округлой «руке»;
  - основания перьев затенены (AO), а само крыло выгнуто назад, как раковина;
  - у демона и дракона перепонки провисают между пальцами и имеют настоящую толщину (лицо, изнанка, кромка), есть суставы, когти и рог;
  - у бабочки толстые крылья с выпуклыми прожилками и объёмное тельце с усиками;
  - на слабых устройствах (FPS Boost) у поверхностей автоматически вдвое меньше рядов.
- **Шрифт HUD:** по умолчанию Montserrat, как у визуальных клиентов. В «Интерфейс → Шрифт HUD» можно выбрать Rubik (мягче) или прежний Inter. Меню остаётся на Inter. Шрифты под SIL OFL 1.1 (`tools/fonts/*-OFL.txt`), пиксельно-чёткие на любом масштабе.
- **Меню открывается там, где его закрыли:** та же вкладка, тот же подраздел (шляпа, крылья, прицел, элемент HUD) и та же высота прокрутки.

## Что нового в 2.13

- **Шляпа закреплена на голове.** Она поворачивается и наклоняется ровно как голова модели (та же точка шеи, тот же
  присед), поэтому больше не «переезжает» на затылок, когда поднимаешь или опускаешь голову.
- **Крылья — 5 видов:** Ангел (перья с цветными кончиками), Демон (перепонки на костях с когтями), Бабочка
  (узор, «глазки», быстрые взмахи), Дракон (большие перепонки, шипы), Феникс (светящиеся перья-пламя с искрами).
  Крепятся к спине и поворачиваются с телом, машут сильнее при ходьбе, прячутся с элитрами; плащ под крыльями
  скрывается. «Эффекты» → «Крылья» → стрелка: переключатель ‹ ›, чипы, размер, сила взмахов, прозрачность, стиль, цвет.
- **Шляпы и крылья видны другим игрокам LavaVisual** — вид и цвет того и другого. Без сервера: через невидимый бит
  скина (кадр ~30 сек, только когда рядом есть игроки, не раньше 10 сек после входа). Теперь включено по
  умолчанию: «Делиться значком, шляпой и крыльями»; чужие можно скрыть — «Шляпы и крылья других игроков».
  Версии 2.12 и 2.13 кадры друг друга просто игнорируют.
- **Модели в 2 раза красивее:** плавное (сглаженное) освещение вместо граней, материалы — металл (золото короны,
  пряжки), драгоценные камни с бликами, мех (Санта, помпоны, ушки), глянец и атлас, мягкая подсветка по краю.
  Новые детали: кисточки у конуса, держава с крестом и мех у короны, бант и пряжка у цилиндра, звёзды и подвеска у
  ведьминой шляпы, конфетти у колпака, золотые кольца на рожках, бантик у ушек, 5 осколков у кристалла, помпоны на сомбреро.
  Шляпы темнеют в пещерах вместе со скином. Всё по-прежнему дёшево: невидимые грани не рисуются, при низком FPS
  детали упрощаются.
- **Новая водянка.** Одна строка, как у визуальных клиентов: тёмная плашка с тонкой рамкой цвета темы, логотип и
  LavaVisual, дальше иконка + значение — ник, FPS, пинг (на сервере, цветом по качеству) и время. Без градиентов.
