...
- **Чёткий шрифт.** Inter Medium/SemiBold растеризуется ровно под пиксели экрана: для каждого
  целого масштаба (1–8) есть свой вариант шрифта, меню и HUD масштабируются только целыми
  пикселями — буквы больше не «мылятся» и не рвутся.
- **Иконки Lucide** в боковом меню, на карточках модулей, кнопках, в HUD (ISC).
- **Клавиши** переделаны: отдельные кнопки W/A/S/D, ЛКМ/ПКМ с настоящим CPS (считаются все
  нажатия, а не только удары), пробел, плавная подсветка нажатия.
- **Броня**: сами предметы брони, полоса и процент прочности, силуэты пустых слотов.
- **Target HUD**: живая 3D-модель моба (исправлен пустой аватар), сердце/щит/дистанция,
  поглощение, полоса экипировки из 5 слотов под панелью.
- **Водянка** (как в Pulse): логотип, место и время, пинг и FPS. Включается в HUD → «Водянка».
- **Звуки**: 27 вариантов (21 собственный синтез + 6 Kenney) для ударов, критов, тотема и
  нового события «Убийство»; переключение стрелками сразу проигрывает звук.
- **Колёсико**: встроенный фильтр теперь гасит серии обратных тиков, двойные срабатывания
  изношенного энкодера и не добавляет задержки обычной прокрутке.
